...
watchEffect(() => {
  document.title = `${data.value?.event ?? "Tech Week"} ${data.value ? data.value.year : ""} - ${data.value ? data.value.location : ""}`
})
</script>
...
<template>
  <div class="mx-auto flex min-h-screen w-full max-w-7xl flex-col gap-6 px-4 py-6 sm:px-6 lg:px-8">
    <header class="w-full">
      ...
      <TitleBlock
        :title="data?.event ?? 'Tech Week'"
        :subtitle="subtitle"
        :bg="data?.bg ?? 'bg-red-600'"
        :text="data?.text ?? 'text-red-600'"
      />
    </header>
